...
watch(() => newPlan.value.collegeId, async (id) => {
  if (!id) return
  courses.value = await request.get(`/courses/college/${id}`)
  classList.value = await request.get(`/classes/college/${id}`)
})

async function load() {
  plans.value = await request.get('/plans')
  colleges.value = await request.get('/colleges')
}

async function createPlan() {
  await request.post('/plans', newPlan.value)
...
async function publishPlan(id) {
  await request.put(`/plans/${id}/publish`)
...
async function batchPublish() {
  ...
  await request.put('/plans/batch-publish', selectedIds.value)
  ElMessage.success('批量发布成功')
  load()
}

async function viewDetail(row) {
  detail.value = await request.get(`/plans/${row.id}`)
