...
async function handleApprove(row) {
  ...
  await request.post('/admin/approvals', { applicationId: row.id, approved: true, comment: '学院审核通过' })
...
async function handleReject(row) {
  ...
  await request.post('/admin/approvals', { applicationId: row.id, approved: false, comment: value || '驳回' })
  ElMessage.success('已驳回')
  load()
}

async function load() {
  allApps.value = await request.get('/admin/applications')
}
