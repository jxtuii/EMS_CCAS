    ...
    <el-card style="margin-bottom:20px">
      ...
      <el-form :inline="true">
        ...
        <el-form-item>
          ...
          <el-button type="warning" plain @click="rollbackScheduling">
            🔄 退回排课
          </el-button>
        </el-form-item>
      </el-form>
...
async function runScheduling() {
  ...
  try {
    result.value = await request.post('/admin/scheduling/run', { semester: semester.value })
...
async function publishTasks() {
  ...
  try {
    publishResult.value = await request.post('/admin/scheduling/publish', { semester: semester.value })
...
async function rollbackScheduling() {
  try {
    await ElMessageBox.confirm(
      '此操作将清空课表和已发布的教学任务，把已排课的计划退回"待教务处排课"状态。确定继续？',
      '退回排课',
      { confirmButtonText: '确定退回', cancelButtonText: '取消', type: 'warning' }
    )
    await request.post('/admin/plans/rollback-scheduling')
    ElMessage.success('已退回：可重新执行排课')
    result.value = null
    publishResult.value = null
    canPublish.value = false
  } catch (e) {
...
onMounted(async () => {
  try { semesters.value = await request.get('/admin/reports/semesters') } catch { semesters.value = ['2025-2026-1'] }
})
</script>
