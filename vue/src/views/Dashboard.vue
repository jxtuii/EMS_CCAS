...
const cards = computed(() => {
  const roles = authStore.user?.roles || []
  const allCards = [
    { icon: 'Document', title: '教学计划', color: '', desc: '管理学期教学计划', show: roles.includes('ADMIN_SCHOOL') },
    { icon: 'Setting', title: '规则配置', color: '', desc: '配置教学限制规则', show: roles.includes('ADMIN_COLLEGE') },
    { icon: 'EditPen', title: '课程申报', color: '', desc: '教师在线申报课程', show: roles.includes('TEACHER') },
    { icon: 'Timer', title: '智能排课', color: '', desc: '自动排课算法', show: roles.includes('ADMIN_SCHOOL') },
  ]
